...
engine = create_engine('sqlite:///m_21_db.db')
...
def received_books():
    author_query = session.query(Author).filter(Author.surname == "Пушкин").subquery()
    books_student_1 = session.query(Book).filter(Book.author_id == author_query.c.id).all()
    student_1_query = session.query(Student).filter(Student.id == 1).one()
    ...
    books_student_2 = session.query(Book).filter(Book.id.in_([4, 5])).all()
    student_2_query = session.query(Student).filter(Student.average_score < 4).first()
